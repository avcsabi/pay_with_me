...
RSpec.describe AuthorizeTransaction, type: :model do
  ...
  describe 'total_captured_amount' do
    it 'returns the correct total taking into account refunds' do
      aut_tr = FactoryBot.create(:authorize_transaction, amount: 100, status: 'captured')
      FactoryBot.create(:capture_transaction, parent_transaction: aut_tr, merchant: aut_tr.merchant, amount: 50)
      cap_tr2 = FactoryBot.create(:capture_transaction, parent_transaction: aut_tr, merchant: aut_tr.merchant, amount: 50, status: 'refunded')
  ...
  describe 'before_validation' do
    it 'sets the default status to pending' do
      aut_tr = AuthorizeTransaction.new
      expect(aut_tr.status).to eq(nil)
      aut_tr.valid?
      expect(aut_tr.status).to eq('pending')
    end
